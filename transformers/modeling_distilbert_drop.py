...
class DistilBertForQuestionAnswering(DistilBertPreTrainedModel):
    def __init__(self, config, max_seq_length, add_sub_max_length):
        ...
        self.classification_3 = nn.Linear(config.dim, 3)                        ### Added
    ...
    def forward(
        ...
        answers_as_add_sub_expressions=None,  ## added
        answers_as_add_sub_numbers=None,      ## added
        add_sub_mask=None,                    ## added
        answers_as_counts=None                ## added
    ):
        r"""
...
#            input_ids=input_ids, attention_mask=attention_mask, head_mask=head_mask)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            loss_fct_counts= CrossEntropyLoss(ignore_index=10)                      ## Added loss function, ignore loss for class 10 which is all numbers != 0...9
            classification_loss = loss_fct_counts(class_logits, answers_as_counts)   ## Added classification loss

            loss_fct_express = CrossEntropyLoss()                                     ### Added loss function
            answer_as_add_sub_expressions_loss = loss_fct_express(add_sub_logits, answers_as_add_sub_expressions)    ### Added

            loss_fct_type = CrossEntropyLoss(ignore_index=2)                       ### Added loss function, ignore loss for dummy (q-span) class
            answer_type_loss = loss_fct_type(answer_type_logits, answer_types)     ### Added
