...
from transformers.modeling_distilbert_drop import DistilBertForQuestionAnswering
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--distilbert_model", default=None, type=str, required=True,
                        help="DistilBert pre-trained model selected in the list: distilbert-base-uncased, "
                        "distilbert-base-uncased-distilled-squad, distilbert-base-cased, distilbert-base-cased-distilled-squad, distilbert-base-german-cased, "
...
####    tokenizer = BertTokenizer.from_pretrained(args.distilbert_model, do_lower_case=args.do_lower_case)
    tokenizer = DistilBertTokenizer.from_pretrained(args.distilbert_model, do_lower_case=args.do_lower_case) #### changed to DistilBertTokenizer
    # Prepare model
    model = DistilBertForQuestionAnswering.from_pretrained(args.distilbert_model, max_seq_length=args.max_seq_length, add_sub_max_length=args.add_sub_max_length,    ### change name and add max_seq_length, add_sub_max_length
    ...
    if args.do_train:
        ...
        cached_train_features_file = args.train_file+'_count_{0}_{1}_{2}_{3}'.format(                            ## Added _count in string
            list(filter(None, args.distilbert_model.split('/'))).pop(), str(args.max_seq_length), str(args.doc_stride), str(args.max_query_length))
        try:
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = DistilBertForQuestionAnswering.from_pretrained(args.output_dir, args.max_seq_length, args.add_sub_max_length)    ### Changed name & Added max_seq_length and add_sub_max_length
        tokenizer = DistilBertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
    else:
        model = DistilBertForQuestionAnswering.from_pretrained(args.distilbert_model, args.max_seq_length, args.add_sub_max_length)    ### Changed name & Added max_seq_length and add_sub_max_length
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        #        test_loss = model(input_ids, segment_ids, input_mask, answers_as_add_sub_numbers=answers_as_add_sub_numbers)
            for i, example_index in enumerate(example_indices):
                ...
                all_results.append(RawResult(unique_id=unique_id,
                                             ...
                                             alt_answer=alt_answer))
                                                                         ### added
    #    writer.add_scalar('Train/Loss', loss.item(), args.num_train_epochs)
    #    writer.add_scalar('Test/Loss', test_loss.item(), args.num_train_epochs)
    #    writer.close()
